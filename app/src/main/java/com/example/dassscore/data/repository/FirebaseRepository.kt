package com.example.dassscore.data.repository
...
class FirebaseRepository {
    ...
    suspend fun signInWithGoogle(idToken: String): Result<User> {
        return try {
            ...
            authResult.user?.let { Result.success(User(it.uid, it.email)) }
                    ?: Result.failure(Exception("Sign in with Google failed: Null user"))
        } catch (e: Exception) {
            Result.failure(e)
        }
    }
            ...
            val results =
                    snapshot.documents.mapNotNull { doc ->
                        DassResult(
                                ...
                                responses =
                                        (doc.get("responses") as? List<Long>)?.map { it.toInt() }
                                                ?: emptyList()
                        )
                    }
    ...
    suspend fun getAllUserProfiles(): Result<Map<String, Map<String, Any>>> {
        return try {
            val snapshot = db.collection("users").get().await()
            val profiles =
                    snapshot.documents.associate { doc -> doc.id to (doc.data ?: emptyMap()) }
            Result.success(profiles)
        } catch (e: Exception) {
            Log.e("FirebaseRepository", "Error getting all user profiles: ${e.message}", e)
            Result.failure(e)
        }
    }
    ...
    suspend fun updateUserProfile(userId: String, profileData: Map<String, Any>): Result<Unit> {
        return try {
            db.collection("users").document(userId).set(profileData, SetOptions.merge()).await()
            Result.success(Unit)
        } catch (e: Exception) {
    ...
    suspend fun saveDassResult(resultToSave: DassResult): Result<Unit> {
        ...
        if (userId.isEmpty()) {
            return Result.failure(
                    IllegalArgumentException("userId in DassResult cannot be empty for saving.")
            )
        }
        return try {
            db.collection("users")
                    .document(userId)
                    .collection("dassResults")
                    .add(resultToSave)
                    .await()
            Result.success(Unit)
        } catch (e: Exception) {
    ...
    suspend fun getUserDassResults(userId: String): Result<List<DassResult>> {
        if (userId.isEmpty()) {
            return Result.failure(
                    IllegalArgumentException("userId cannot be empty for fetching results.")
            )
        }
        return try {
            val querySnapshot =
            ...
            Log.e(
                    "FirebaseRepository",
                    "Error getting user DASS results for $userId: ${e.message}",
                    e
            )
            Result.failure(e)
        }
